...
describe("Login Component", () => {
  ...
  it("calls the presenter's login method with correct parameters when sign-in button is pressed",
    async () => {
      ...
      const { signInButton, aliasField, passwordField, user } =
        renderLoginAndGetElements(originalUrl, loginPresenter);
      ...
      verify(spyLoginPresenter.doLogin(alias, password, false, originalUrl)).once();
    });
});
